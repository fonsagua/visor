...
Take care. This lib has been hacked by iCarto
...
L.Control.GroupedLayers = L.Control.extend({
  ...
  _getLayer: function (id) {
    ...
    // for (var i = 0; i < Object.keys(this._layers).length; i++) {
  ...
  _addItem: function (obj) {
    ...
    if (obj.overlay) {
      ...
      input.id = '_leaflet_id' + L.stamp(obj.layer);
    } else {
  ...
  _onInputClick: function (myevent) {
    var i, input, obj,
    ...
    var layerId = myevent.currentTarget.id.replace('_leaflet_id', '')
    var checked = myevent.currentTarget.checked
    var isImageOverlay = myevent.currentTarget.nextSibling.firstElementChild.className === 'image-overlay-layer';
    if (isImageOverlay) {
        // console.log('here')
        obj = this._getLayer(layerId);
        ...
        if (myevent.currentTarget.type === "radio") {
            var foo = document.getElementsByName(myevent.currentTarget.name);
            foo.forEach(function(v) {
                var layerId2 = v.id.replace('_leaflet_id', '')
                if (layerId2 != layerId) {
                    feature_group.removeLayer(self._layers[layerId2].layer);

                }
            })
        }
        restackOneLayer({id:layerId, visible:checked});
    }
